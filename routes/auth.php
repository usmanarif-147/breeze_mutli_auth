...
Route::middleware('guest')->group(function () {
    ...
    Route::get('forgot-password', [PasswordResetLinkController::class, 'createStudent'])
        ->name('student.password.request');

    Route::post('forgot-password', [PasswordResetLinkController::class, 'storeStudent'])
        ->name('student.password.email');

    Route::get('reset-password/{token}', [NewPasswordController::class, 'createStudent'])
        ->name('student.password.reset');

    Route::post('reset-password', [NewPasswordController::class, 'storeStudent'])
...
Route::middleware(['student'])->group(function () {
    ...
    Route::post('email/verification-notification', [EmailVerificationNotificationController::class, 'storeStudent'])
        ->middleware('throttle:6,1')
        ->name('student.verification.send');

    Route::get('confirm-password', [ConfirmablePasswordController::class, 'showStudent'])
        ->name('student.password.confirm');

    Route::post('confirm-password', [ConfirmablePasswordController::class, 'storeStudent'])
        ->name('student.password.confirm.store');

    Route::put('password', [PasswordController::class, 'update'])
        ->name('student.password.update');

    Route::post('logout', [AuthenticatedSessionController::class, 'destroyStudent'])
        ->name('student.logout');
});
